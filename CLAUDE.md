...
- **Pages:**
  ...
  - `src/pages/ea/gdbasket.astro` — gdBasket detail สไตล์ **github-repo** (repo header + toolbar + file list + README + sidebar). ปุ่มโหลด absolute -> `intensivetrader.com/d/gdbasket`. **rework 2026-07-10: copy ทั้งหน้าเป็นภาษาผู้ใช้ (source of truth = `C:/WORKSPACE/.claude/artifacts/gdbasket-copy-mockup.md` — tagline "Adaptive Buy Grid Behavior EA", ห้าม logic ภายในหลุด) + Engine Profile แท็บสลับ 3 โหมด `showEngine` (strada+ / sport+ / VELOCE default·แนะนำ — ตัด CORSA ออกจากช่องเลือก, rework v3.02 VELOCE 2026-07-23) + section ผลทดสอบ VELOCE (v3.02 rework): KPI row 4 ช่อง + Copy Trade block (ลิงก์ social.tp-redirect + risk warning) + popup 'ดูสถิติเต็ม' (7 กลุ่ม + WFA 25 รูปแบบ + survival) + ตารางผล VELOCE (net 250,351.30 / ล้าง 4 จาก 31 รอบ / survival 87-87-100-100-100) + 'กว่าจะมาเป็น VELOCE' 5 ขั้น — หน่วย "รอบ" (Capital cycle) + Releases sidebar เป็น widget สไตล์ GitHub ชี้ `/ea/gdbasket/changelog` (route Astro ใหม่ 2026-07-23 `src/pages/ea/gdbasket/changelog.astro` — ย้ายจาก static `public/gdbasket-changelog.html` ที่ลบแล้ว + redirect 301 ใน vercel.json) = github-releases-style hardcode รุ่น publish (v3.03 latest / v3.02 VELOCE / v2.02 / v2.01 / v2.00 / v1.37 / v1.18) + ตัดบล็อก Languages.** **sidebar = About + Trading Journal card (ดึง Supabase `baskets` client-side: กำไรสะสม/winrate/count + toggle รวม/ต่อพอร์ตจาก `account_login`) + Myfxbook card (realtime ผ่าน proxy, SSR loop ทุกบัญชี — การ์ดพอร์ต 2 gdbasket lx ขึ้นเองเมื่อ API คืน — ดู memory `reference_room_gdbasket_web`) + Releases + Quick links (anchor #readme/#modes/#install/#changelog) + Topics chips + ผู้พัฒนา.** _(UI port มาตรฐาน artifact `repo-*`/`readme-*`/`resource-*` class system 2026-07-16 — ปุ่มโหลดมีที่ toolbar จุดเดียว ไม่มีปุ่มโหลดซ้ำใน sidebar)_ มี **XM affiliate 3 จุด** (การ์ด + แถว Broker ใน About + ขั้น 0 วิธีติดตั้ง) + **VPS affiliate card** (ท้ายขั้นตอนติดตั้ง ก่อน section "คำแนะนำ" — ลิงก์ `bit.ly/MTCloudVPS`, const `VPS_AFF`) + Trading Journal 3 จุด (tab + สารบัญ + การ์ด) ลิงก์ตรง `journal.intensivetrader.com`. ภาพ chart จริงหัว readme
...
- **อัปเดต gdBasket (version/คำโปรย) ต้องแก้ให้ครบทุกจุด** — version string + copy กระจายหลายไฟล์ อัปที่เดียวไม่พอ (เคยตกหล่น: การ์ด home ค้าง v2.00 หลังหน้า detail ไป v2.01 แล้ว, 2026-07-11):
  ...
  2. `src/pages/ea/gdbasket.astro` — หน้า detail: version badge หลายจุด (repo header / toolbar / commit bar) + `og:title`/`description` + `ogImageWidth/Height` (tag Layout) + เนื้อหา (source of truth copy = `C:/WORKSPACE/.claude/artifacts/gdbasket-copy-mockup.md` — ห้าม logic ภายในหลุด เช่น ATR/EMA)
  3. `src/pages/ea/gdbasket/changelog.astro` — changelog route (hardcode releases — เพิ่ม block รุ่นใหม่บนสุด; ย้ายจาก static `public/gdbasket-changelog.html` 2026-07-23 + redirect 301 vercel.json)
